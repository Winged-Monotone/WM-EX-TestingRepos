...
namespace XRL.World.Parts.Mutation
{
    ...
    public class SerpentineForm : BaseMutation
    {
        ...
        public int TailCheckCycling = 1200;
        ...
        public override bool Mutate(GameObject GO, int Level)
        {
            ...
            {
                XRL.Core.XRLCore.Core.Game.PlayerReputation.modify("Unshelled reptiles", 100, true);
                XRL.Core.XRLCore.Core.Game.PlayerReputation.modify("Apes", -200, true);
        ...
        public override bool ChangeLevel(int NewLevel)
        {
            ...
            var mBody = ParentObject.GetPart<Body>();
            BodyPart firstPart = this.ParentObject.GetPart<Body>().GetFirstPart(this.BodyPartType);
            ...
            if (NewLevel < 1 && this.SerpentileTail != null)
            {
                if (!this.SerpentileTail.HasPart("Armor"))
                {
                    this.SerpentileTail.AddPart<Armor>();
                }
        ...
        public override bool FireEvent(Event E)
        {
            ...
            else if (E.ID == "EndTurn")
            {
                if (TailCheckCycling > 0)
                {
                    ...
                    if (TailCheckCycling <= 0)
                    {
                        BodyPart firstPart = this.ParentObject.GetPart<Body>().GetFirstPart(this.BodyPartType);
                        SerpentileTail = GameObject.create("Serpentine Tail");
                        if (firstPart == null || firstPart.Equipped != SerpentileTail)
                        {
                            firstPart.Equipped?.UnequipAndRemove();
                            firstPart.Equip(SerpentileTail, Forced: true);
                        }
                    }

                    TailCheckCycling = 600;
                }
                try
                {
            ...
            else if (E.ID == "CommandConstrict")
            {
                ...
                {
                    GameObject combatTarget = cell.GetCombatTarget(Attacker: ParentObject);
