...
namespace XRL.World.Parts.Mutation
{
    ...
    public class GelatinousFormAcid : BaseDefaultEquipmentMutation
    {
        ...
        public Guid WMAcidSpitAbilityID = Guid.Empty;
        public int Damage = 2;
        public int Duration;
                ...
                if (ParentObject.CurrentCell != null && parameter.Amount != 0)
                {
                    ...
                    foreach (Cell cell in adjacentCells1)
                    {
                        if (!cell.IsOccluding() && Stat.Random(1, 100) <= 10 + (5 * Level / 2))
                        {
                            ...
                            cell.AddObject(Object: AcidContainer,
                                            Forced: true,
                                            System: false,
                                            IgnoreGravity: false,
                                            NoStack: false);
            ...
            else if (E.ID == "CommandSpitAcid")
            {
                if (!this.ParentObject.pPhysics.CurrentCell.ParentZone.IsWorldMap())
                {
                    ...
                    if (list != null)
                    {
                        ...
                        CooldownMyActivatedAbility(WMAcidSpitAbilityID, 40);
                        int num = 0;
        ...
        public override bool Mutate(GameObject GO, int Level)
        {
            ...
            AddAcidGlobul();
            WMAcidSpitAbilityID = AddMyActivatedAbility(Name: "Spit Acid", Command: "CommandSpitAcid", Class: "Physical Mutation", Icon: "*");
        ...
        {
            ParentObject.RemovePart<AcidImmunity>();
            ParentObject.RemovePart<WaterHazardous>();
            ParentObject.RemovePart<DamagesArmorOnEquipped>();
