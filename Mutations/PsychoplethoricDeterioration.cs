...
namespace XRL.World.Parts.Mutation
{
    ...
    public class PsychoplethoricDeterioration : BaseMutation
    {
        ...
        public Guid SoulShuntAbilityID;
        public bool EsperCheck = false;
        ...
        public void SoulShunt()
        {
            ...
            if (TargetHusk != null)
            {
                ...
                if (!TargetHusk.MakeSave("Willpower", 8 + LevelDifference, ParentObject, "Ego", ParentObject.It + " attempted to shunt " + TargetHusk.Its + " mind from " + TargetHusk.Its + " body.", false, false, false, false))
                {
                    ...
                    var AncestrallyGifted = false;

                    if (OwnersMutationAlterations.HasMutation("Animancy"))
                    {
                        AncestrallyGifted = true;
                        if (AncestrallyGifted)
                        {
                            TargetsMutationAlterations.AddMutation("Animancy", 1);
                        }
                    }
                    ...
                    foreach (var key1 in OwnersMentalMutations)
                    {
                        OwnersMutationAlterations.RemoveMutation(key1);
                    }
        ...
        public override bool FireEvent(Event E)
        {
            ...
            {
                // ActivatedAbilities SoulShuntAbilityID = ParentObject.GetPart("ActivatedAbilities") as ActivatedAbilities;
                // SoulShuntAbilityID.GetAbility(ActivatedAbilityID).Cooldown = 24000;
            ...
            {
                CooldownMyActivatedAbility(SoulShuntAbilityID, 2400, ParentObject);
                ...
                if (EsperCheck == true)
                {
                    if (!ParentObject.IsEsper())
                    {
                        ParentObject.SetStringProperty("Genotype", "Esper");
                        ParentObject.AddPart<Esper>();
                    }
                }
                ...
                if (OriginalBody != null)
                {
                    ...
                    {
                        ParentObject.DisplayName = OriginalBody.DisplayNameOnlyDirect;
                        OriginalBody.DisplayName = Names.NameMaker.MakeName(ParentObject);
                    }
                    else
                    {
                        var NewName = ParentObject.DisplayNameOnlyDirect;
                        ParentObject.DisplayName = OriginalBody.DisplayNameOnlyDirect;
