...
namespace XRL.World.Effects
{
    ...
    public class Darkening : Effect
    {
            ...
            try
            {
                ...
                foreach (var Cell in AdjacentCells)
                {
                    if (!Cell.HasObject("MagicalDarkness"))
                    {
                        ...
                        int Ran = Stat.Random(0, 3);
                        Darkness.Duration = ((UmbralHook.Level * (1 + ParentsEgoModifer) / 2) + Ran);
        ...
        public override bool FireEvent(Event E)
        {
            ...
            else if (E.ID == "ClimbUp")
            {
                Cell C = The.Player.CurrentCell;
                if (C.ParentZone.Z <= 10 && !C.ParentZone.IsInside() && !C.ParentZone.IsWorldMap() && Object.IsPlayer())
                {
                    AddPlayerMessage("You cannot enter the world-map while you have the darkening status effect.");
                    return false;
                }
            }
        ...
        {
            base.PlayWorldSound("DarknessShroud", 0.5f, 0, true, null);
